...
class Configuration:
    ...
    def we_can_add_piece_to_existent_configuration(self, lego_brick, piece_info_in_space):
        ...
        number_of_studs_that_connects_a_tube = len(
            self.get_studs_which_connects_tubes_for_piece(piece_info_in_space, lego_brick.brick_id))
    ...
    def add_piece_to_existent_configuration(self, lego_brick, start_coordinates, piece_info_in_space):
        ...
        all_studs_with_connection_info = self.get_studs_which_connects_tubes_for_piece(piece_info_in_space,
                                                                                       lego_brick.brick_id, 1)
...
def config1_pieces1():
    ...
    print(configuration.place_in_tubes(Brick(3003, "White", configuration), [3, 3, 0], rotation=0))
    print(configuration.place_in_studs(Brick(3003, "White", configuration), [3, 5, 0], rotation=0))
...
def get_the_pieces_selected_by_the_user():
    # id, numar
    user_pieces = [[2357, 2], [3004, 2], [3006, 6], [3010, 1], [3710, 2], [4490, 1]]
    return user_pieces
...
def eliminate_special_pieces_from_config(config, disponible_pieces):
    ...
    li_id_for_special_pieces_global = [4286, 2431, 4490]  # DE COMPLETAT - VA FI O VARIABILA GLOBALA
...
def get_possible_start_positions_for_pieces(config):
    possible_start_positions = []
    for key, value in config.occupied_space.items():
        for i in value:
            possible_start_positions.append([i[0], i[1], i[2], 0])
    return possible_start_positions


def get_first_coord_unassigned_from(li_xyz):
    for i in li_xyz:
        if li_xyz[4] == 0:
            return li_xyz[:3]
    return None
...
def we_can_remove_piece_from_config(config, piece_info_in_space):
    return True # de implementat -> asta trebuia sa fie facuta de tiberiu....... .....................


def remove_piece_from_config_from_coords(config, piece_info_in_space):
    print("removed")
    return config
...
def get_updated_li_xyz(li_xyz, piece_info_in_space, flag_piesa_adaugata=True):
    return li_xyz.copy()


def backtracking(li_xyz, config, disponible_pieces):
    # pasul din bkt va fi definit ca prima pozitie din stanga jos ce nu a fost acoperita
    step = get_first_coord_unassigned_from(li_xyz)
    for disponible_piece_id, number_of_disponible_pieces in disponible_pieces:
        possible_rotations_for_piece = [0, 1, 2, 3]  # la unele piese vor fi mai putine rotatii, REV, se poate reduce de 4 ori complexitatea
        for rotation in possible_rotations_for_piece:
            piece_info_in_space = calculates_coordinates_starting_from_the_beginning(disponible_piece_id, step, rotation)
            # if(validare())
            if we_can_remove_piece_from_config(config, piece_info_in_space):
                config = remove_piece_from_config_from_coords(config, piece_info_in_space)
                disponible_pieces, _ = remove_piece_from_pieces_list(disponible_pieces, disponible_piece_id)
                li_xyz = get_updated_li_xyz(li_xyz, piece_info_in_space, False)
                # if(solutie())
                if config_is_empty():
                    return True
                else:
                    backtracking(get_updated_li_xyz(li_xyz, piece_info_in_space), config, disponible_pieces)
                config = config.add_piece_to_existent_configuration(config, piece_info_in_space)
                disponible_pieces = add_piece_to_pieces_list(disponible_pieces, disponible_piece_id)
                li_xyz = get_updated_li_xyz(li_xyz, piece_info_in_space, True)
...
def get_all_configurations_that_we_can_do():
    ...
    for config_index, config in enumerate(configurations_to_verify):
        ...
        config, disponible_pieces, we_have_special_pieces = eliminate_special_pieces_from_config(config, disponible_pieces)

        if we_have_special_pieces:
            li_xyz = get_possible_start_positions_for_pieces(config)
            if backtracking(li_xyz, config, disponible_pieces):
...
if __name__ == '__main__':
    ...
    print("Configuratiile posibile", get_all_configurations_that_we_can_do(), sep=": ")
